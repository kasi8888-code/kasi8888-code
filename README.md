<div align="center">
  <h1>Hi there, I'm Naveen Kasi 👋</h1>
  <h3><i>Bridging the gap between Software Development and Artificial Intelligence</i></h3>
</div>

---

### 👨‍💻 About Me

I am a developer and researcher focused on building intelligent systems that solve real-world problems. My work spans **Applied AI**, **IoT Hardware**, and **Decentralized Applications**.

- 🔭 **I'm currently working on:** *ECG Arrhythmia Detection System: A Hybrid Deep Learning Approach for Clinical Decision Support*
- 👯 **I'm looking to collaborate on:** Open Source AI Agents & Medical Imaging Research
- ⚡ **Fun fact:** I've built tech across three completely different domains: a concrete humidity sensor, an NFT marketplace, and an AI recruitment bot.

---

<div align="center">
  <h3>🛠️ Technologies & Tools</h3>

  <img src="https://img.shields.io/badge/-Python-3776AB?style=for-the-badge&logo=python&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" height="28" />
  <img src="https://img.shields.io/badge/-Solidity-363636?style=for-the-badge&logo=solidity&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" height="28" />

  <br/>
  <img src="https://img.shields.io/badge/-TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-LangChain-1C3C3C?style=for-the-badge&logo=chainlink&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white" height="28" />

  <br/>
  <img src="https://img.shields.io/badge/-Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-Google_Cloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white" height="28" />
  <img src="https://img.shields.io/badge/-Git-F05032?style=for-the-badge&logo=git&logoColor=white" height="28" />
</div>

---


### 🏆 Featured Projects

#### 🤖 [Agent M - Open Source AI Platform](https://github.com/ak-1344/AgentM)
*Automated Professional Outreach with Generative AI*
- **Role:** Open Source Contributor (Frontend/AI Integration)
- **Tech:** Python, FastAPI, LangChain, Gemini 1.5 Pro
- **Impact:** Implemented RAG pipelines to parse user resumes and generate context-aware emails, automating the job application workflow.

#### 💎 [MetaLease - NFT Rental Marketplace](https://github.com/kasi8888-code/MetaLease)
*Decentralized Asset Lending Protocol*
- **Tech:** Solidity, Hardhat, ERC-4907 Standard
- **Impact:** Built a trustless rental system on the Sepolia testnet that allows users to rent NFTs without collateral using the `userOf` standard.

#### 📱 [MedTag - NFC Drug Authentication](https://github.com/kasi8888-code/MedTag)
*Anti-Counterfeit Solution using IoT*
- **Tech:** Android (Kotlin), Host Card Emulation (HCE), ISO-DEP
- **Impact:** Developed a "digital handshake" protocol that turns smartphones into virtual smart cards to verify drug authenticity offline.

---

<div align="center">
  <a href="www.linkedin.com/in/naveen-kasi-59244a322">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="naveenkasi.vit@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</div>
